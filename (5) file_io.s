...
out_file: .ascii "uppercase.txt\0" # see line 95
...
# storage for our file descriptors
.lcomm FD_IN, 4   # input  fd
.lcomm FD_OUT, 4  # output fd
...
store_fd_in: # store file descriptor for input file
    movl %eax, FD_IN #save the given file descriptor
...
open_fd_out:
    ...
    # movl $out_file, %ebx #output filename into %ebx
...
store_fd_out: # store file descriptor for output file
    movl %eax, FD_OUT #store the file descriptor here

###BEGIN MAIN LOOP###
read_loop_begin:
    ###READ IN A BLOCK FROM THE INPUT FILE###
    movl $SYS_READ, %eax
    movl FD_IN, %ebx #get the input file descriptor
